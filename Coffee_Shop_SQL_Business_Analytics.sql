...
SELECT staff_id, first_name, last_name, total_worked_hours,
		...
		DENSE_RANK() OVER( ORDER BY total_worked_hours DESC) rank_top_working_employees
...
SELECT 
    EXTRACT(HOUR FROM o.created_at::TIMESTAMP) AS busiest_hour,
...
SELECT EXTRACT(MONTH FROM CAST(o.created_at AS TIMESTAMP)) AS month,   --OR USE ::TIMESTAMP
...
FROM(
SELECT CASE WHEN EXTRACT(HOUR FROM created_at::TIMESTAMP) BETWEEN 5 AND 10 THEN 'Morning'
			WHEN EXTRACT(HOUR FROM created_at::TIMESTAMP) BETWEEN 12 AND 15 THEN 'Afternoon'
			WHEN EXTRACT(HOUR FROM created_at::TIMESTAMP) BETWEEN 17 AND 19 THEN 'Evening'
			ELSE 'Other' END AS order_period,
...
ORDER BY order_period = 'Morning' DESC, order_period = 'Afternoon' DESC, order_period = 'Evening' DESC,
		 order_period = 'Other' DESC, order_period = 'Total'
...
SELECT mi.item_name, o.item_id, SUM(o.quantity) AS quantity_sold,
		...
		CASE WHEN SUM(quantity) <5 THEN 'Very Low Sales'
				WHEN SUM(quantity) <= 10 AND SUM(o.quantity * mi.item_price) >40 THEN 'High Prices Items'
...
WITH ing_quantity AS(
			SELECT ing.ing_id, ing.ing_name, ing.ing_weight, ing_meas AS measurement, SUM(inv.quantity) AS quantity_left
			FROM ingredients ing
			JOIN inventory inv ON ing.ing_id = inv.ing_id
			GROUP BY ing.ing_id, ing.ing_name, ing.ing_weight, measurement)
...
SELECT mi1.item_name || '->' || mi2.item_name || '->' || mi3.item_name AS item_chain, COUNT(*) AS frequency, 2 AS chain_lenmgth
FROM orders o1
JOIN orders o2 ON o1.order_id = o2.order_id AND o1.item_id < o2.item_id
...
FROM orders
GROUP BY cust_name
HAVING COUNT(*) >= 10 AND COUNT(DISTINCT CAST(created_at AS DATE)) >= 5  --cast
ORDER BY distinct_order_days DESC

-- Q20: Which menu items are most popular by time of day (morning, afternoon, evening)?
SELECT t.time_of_day, t.item_id, COALESCE(mi.item_name,'No Data') AS item_name, t.order_count
FROM
	(SELECT o.item_id, CASE WHEN EXTRACT(HOUR FROM created_at::TIMESTAMP) BETWEEN 5 AND 11 THEN 'Morning'
	...
	COUNT(*) AS order_count,
	ROW_NUMBER() OVER(PARTITION BY CASE WHEN EXTRACT(HOUR FROM created_at::TIMESTAMP) BETWEEN 5 AND 11 THEN 'Morning'
							WHEN EXTRACT(HOUR FROM created_at::TIMESTAMP) BETWEEN 12 AND 16 THEN 'Afternoon'
							WHEN EXTRACT(HOUR FROM created_at::TIMESTAMP) BETWEEN 17 AND 20 THEN 'Evening' END
